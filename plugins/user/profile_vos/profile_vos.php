 ...
 * @since       1.6
 */
class PlgUserProfile_vos extends JPlugin {
	/**
	 * Date of birth.
	 * @var    string
	 * @since  3.1
	...
	public function onContentPrepareData ($context, $data) {
		...
		if (is_object($data)) {
			...
			if (!JHtml::isRegistered('users.mailtype')) {
				JHtml::register('users.mailtype', array(__CLASS__, 'mailtype'));
			}
	...
	/**
	 * returns langstring for mailtype
	 * @param   string $value url to use
	 * @return mixed|string
	 */
	public static function mailtype ($value) {
		if (empty($value)) {
			return JHtml::_('users.value', $value);
		} else {
			return JText::_('PLG_USER_BIXMAILINGSETTINGS_MAILTYPE_' . strtoupper($value));
		}
	}
	...
	public function onContentPrepareForm ($form, $data) {
		...
		include_once(JPATH_ROOT . '/administrator/components/com_bixmailing/classes/bixtools.php');
		...
		if ($name == 'com_users.registration') {
			foreach ($form->getFieldset('bixmailingsettings') as $field) {
				$form->removeField($field->fieldname, 'profile');
			}
		}

		BixTools::uikit();
		BixTools::assetJs('bixuserform');
	...
	public function onUserAfterSave ($data, $isNew, $result, $error) {
		...
		if ($userId && $result && isset($data['profile']) && (count($data['profile']))) {
			try {
				unset($data['profile']['activationKey']); //only set in activation script
				//data is niet altijd compleet ivm gebruikersrechten
				$currentProfile = BixmailingHelper::getUserProfile($userId)->toArray();
				$data['profile'] = array_merge($currentProfile, $data['profile']);
				// Sanitize the date
				$data['profile']['dob'] = $this->date;
				//modified
				$data['profile']['modified'] = JFactory::getDate()->toSql();

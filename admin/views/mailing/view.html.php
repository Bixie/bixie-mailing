 ...
 * View to edit
 */
class BixmailingViewMailing extends JViewLegacy {
	protected $state;
	protected $item;
	protected $form;
	protected $canDo;
	...
	protected function addToolbar () {
		...
		JToolBarHelper::title('<span class="color">' . JText::_('COM_BIXMAILING') . '</span> :: ' . JText::_('COM_BIXMAILING_MAILING') . $title, 'mailing');
		...
		if (!$checkedOut && ($this->canDo->get('core.edit') || ($this->canDo->get('core.create')))) {
			JToolBarHelper::apply('mailing.apply', 'JTOOLBAR_APPLY');
			JToolBarHelper::save('mailing.save', 'JTOOLBAR_SAVE');
		}
		if (!$checkedOut && ($this->canDo->get('core.create'))) {
			JToolBarHelper::custom('mailing.save2new', 'save-new.png', 'save-new_f2.png', 'JTOOLBAR_SAVE_AND_NEW', false);
		}
		// If an existing item, can save to a copy.
		if (!$isNew && $this->canDo->get('core.create')) {
			// JToolBarHelper::custom('mailing.save2copy', 'save-copy.png', 'save-copy_f2.png', 'JTOOLBAR_SAVE_AS_COPY', false);
		}
		if (empty($this->item->id)) {
			JToolBarHelper::cancel('mailing.cancel', 'JTOOLBAR_CANCEL');
		} else {
			JToolBarHelper::cancel('mailing.cancel', 'JTOOLBAR_CLOSE');
		}
